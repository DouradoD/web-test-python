...
def _filter(members):
    dict_ = {}
    for m in members:
        name = getattr(m, 'page_name', None) or getattr(m, 'mapping_page')
...
class Pages:
    ...
    def __init__(self, session_info, **injection_objects):
    ...
    def load_pages(self, **injection_objects):
        pages = []
        mappings = []
        pages_folder = f'mobile.{self._session_info.platform}' if self._session_info.platform in ['android',
                                                                                                  'ios'] else 'web'
        package = import_module(f'project.{pages_folder}')
            ...
            if not module_info.ispkg:
                ...
                for _, member in getmembers(module, lambda m: (isclass(m) and m.__module__ == module_info.name)
                                                              and (hasattr(m, 'page_name')
                                                                   or hasattr(m, 'mapping_page'))):
                    if hasattr(member, 'page_name'):
                        pages.append(member)
                    if hasattr(member, 'mapping_page'):
