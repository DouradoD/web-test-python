from tests.config.utils.utils import LoadCmdLineCapabilities
from tests.config.plugins.hooks import Hooks
...
def pytest_addoption(parser):
    ...
    group.addoption(
        '--browserstack', action='store_true', help='Run tests by using browserstack platform.'
    )
    group.addoption(
...
def pytest_configure(config):
    ...
    # Ignoring mark warnings as we are using marks to filter the tests and there is no need to track them all
